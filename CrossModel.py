...
def createSimpleModel(allFeatures,allInputs):
    ...
    for units in hidden_units:
        layer1 = layers.Dense(units=500,activation='relu')(layer1)
        layer1 = layers.Dense(units=100,activation='softmax')(layer1)
        layer1 = layers.Dense(units=50)(layer1)
        layer1 = layers.ReLU()(layer1)
        layer1 = layers.Dropout(dropout_rate)(layer1)

    layer2 = allFeatures
    for units in hidden_units:
        layer2 = layers.Dense(units=500,activation='relu')(layer2)
        layer2 = layers.Dense(units=100,activation='softmax')(layer2)
        layer2 = layers.Dense(units=50)(layer2)
        layer2 = layers.ReLU()(layer2)
        layer2 = layers.Dropout(dropout_rate)(layer2)

    merged = layers.concatenate([layer1, layer2])
    outputs = layers.Dense(units=4, activation="softmax")(merged)
